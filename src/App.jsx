...
function AuthOptions({ setAuthError, authError }) {
  ...
  async function continueWithTelegram() {
    if (!supabase) { setAuthError("Supabase is not configured in this build."); return; }
    const provider = import.meta.env.VITE_TELEGRAM_AUTH_PROVIDER;
    if (!provider) { setAuthError("Telegram sign-in is not configured yet."); return; }
    setBusy("telegram"); setAuthError("");
    const { error } = await supabase.auth.signInWithOAuth({
      provider,
      options: { redirectTo: window.location.origin }
    });
    if (error) { setBusy(""); setAuthError(error.message || "Telegram sign-in could not start."); }
  }
